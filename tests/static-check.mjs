...
for(const marker of ['pwa-install-btn','installCustomerPwa','yummypro_last_restaurant','/manifest.webmanifest?v=1625','/pwa-sw.js','yummypro_client_pwa_identity_v1622','customer-pwa-v1623','Versión v1.6.56'])if(!html.includes(marker))throw new Error('Falta PWA cliente actual: '+marker);
...
for(const marker of ['yummypro-client-v1632-fulfillment-cards','SKIP_WAITING','/offline.html','/icon-192.png','/icon-512.png'])if(!pwaSw.includes(marker))throw new Error('Service Worker PWA cliente v1.6.25 incompleto: '+marker);
...
for(const marker of ['business_public_client_mode','effective_demo','effective_demo','Esta es una cuenta demo','renderDemoOrders','professional-demo-mp','Pago online seguro'])if(!html.includes(marker))throw new Error('Falta modo demo efectivo controlado por credenciales del administrador: '+marker);

for(const marker of ['activate_admin_client_preview','has_admin_client_preview','admin_client_token_hash','Admin prueba','__adminClientPreview'])if(!html.includes(marker))throw new Error('Falta sesión automática del administrador en menú cliente: '+marker);

for(const marker of ['menu-mobile-card-fix-v1655','CONFIG.fulfillment','fulfillmentAllowed','applyFulfillmentAvailability','pickup_enabled','delivery_enabled'])if(!html.includes(marker))throw new Error('Falta Retiro/Delivery o fix móvil: '+marker);
