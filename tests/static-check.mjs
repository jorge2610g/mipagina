...
for(const marker of ['pwa-install-btn','installCustomerPwa','yummypro_last_restaurant','/manifest.webmanifest?v=1625','/pwa-sw.js','yummypro_client_pwa_identity_v1622','customer-pwa-v1623','Versión v1.6.51'])if(!html.includes(marker))throw new Error('Falta PWA cliente actual: '+marker);
...
for(const marker of ['business_payment_public_status','effective_demo','using_admin_credentials','Esta es una cuenta demo','renderDemoOrders','professional-demo-mp','Pago online seguro'])if(!html.includes(marker))throw new Error('Falta modo demo efectivo controlado por credenciales del administrador: '+marker);
