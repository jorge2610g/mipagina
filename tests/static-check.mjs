...
for(const marker of ['pwa-install-btn','installCustomerPwa','yummypro_last_restaurant','/manifest.webmanifest?v=1622','/pwa-sw.js','yummypro_client_pwa_identity_v1622','customer-pwa-v1623','Versión v1.6.27'])if(!html.includes(marker))throw new Error('Falta PWA cliente actual: '+marker);
...
for(const marker of ['yummypro-client-v1627-professional-dark','SKIP_WAITING','/offline.html','/icon-192.png','/icon-512.png'])if(!pwaSw.includes(marker))throw new Error('Service Worker PWA cliente v1.6.25 incompleto: '+marker);
...
for(const marker of ['professional-booking-app','professional-booking-mode','get_public_professional_catalog','get_professional_public_catalog','get_professional_available_slots','create_public_professional_appointment','renderProfessionalPublicBooking','professional_service_selected','professional_booking_created','create-professional-appointment-payment','yummypro_professional_payment_return','get_public_professional_appointment_status','professional-deposit-pay','mercadopago_configured',':root[data-theme="dark"] .professional-booking',':root[data-theme="dark"] .professional-booking-card',':root[data-theme="dark"] .professional-choice','professional-theme'])if(!html.includes(marker))throw new Error('index.html: falta reserva pública profesional '+marker);
