const YUMMYPRO_CLIENT_CACHE="yummypro-client-app-v1618";
const CLIENT_CORE=["/offline.html","/yummypro-client.webmanifest","/icon-192.png","/icon-512.png","/apple-touch-icon.png"];
self.addEventListener("install",event=>{
 event.waitUntil(caches.open(YUMMYPRO_CLIENT_CACHE).then(c=>c.addAll(CLIENT_CORE)).then(()=>self.skipWaiting()));
});
self.addEventListener("activate",event=>{
 event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("yummypro-client")&&k!==YUMMYPRO_CLIENT_CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});
